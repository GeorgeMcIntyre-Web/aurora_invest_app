...
export function TechnicalsCard({ stock, technicalView }: TechnicalsCardProps) {
  ...
  const indicators = [
    { label: 'Current Price', value: `$${price?.toFixed?.(2) ?? '0.00'}` },
    { label: 'SMA 20', value: sma20 ? `$${sma20.toFixed(2)}` : 'N/A' },
    { label: 'SMA 50', value: sma50 ? `$${sma50.toFixed(2)}` : 'N/A' },
    { label: 'SMA 200', value: sma200 ? `$${sma200.toFixed(2)}` : 'N/A' },
    { label: 'RSI (14)', value: rsi ? rsi.toFixed(1) : 'N/A' },
