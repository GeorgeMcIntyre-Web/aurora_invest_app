...
export function HistoricalCard({ data, selectedPeriod, isLoading = false }: HistoricalCardProps) {
  const summary = useMemo(() => {
    ...
    return {
      periodReturn: calculateReturns(data).period,
      annualizedReturn: calculateReturns(data).annualized,
  ...
  if (data?.dataPoints?.length) {
    observations.push(
      `Price ${summary.periodReturn >= 0 ? 'increased' : 'decreased'} ${formatPercent(
        Math.abs(summary.periodReturn)
      )} over the last ${PERIOD_LABEL[selectedPeriod] ?? 'period'}.`
    );
  }
  ...
  if (summary.trend === 'uptrend') {
    ...
    observations.push('Selling pressure is dominant; rallies have been short-lived.');
  } else {
    observations.push('Price action is choppy with no clear direction.');
  }
