...
const severityStyles: Record<
  Severity,
  { text: string; bg: string; bar: string; badge: string; label: string }
> = {
  low: {
    text: 'text-emerald-400',
    bg: 'bg-emerald-500/15 border border-emerald-500/30',
    bar: 'from-emerald-400 to-emerald-500',
    badge: 'bg-emerald-500/10 border border-emerald-400/30',
    label: 'Lower Risk',
  },
  medium: {
    ...
    badge: 'bg-yellow-500/10 border border-yellow-500/30',
    label: 'Moderate Risk',
  },
  high: {
    text: 'text-red-400',
  ...
  unknown: {
    ...
    badge: 'bg-ai-bg border border-gray-700',
    label: 'No Signal',
  },
};
...
const categorizeVolatility = (pct?: number): Severity => {
...
const categorizeSentiment = (sentimentView?: string, consensus?: string): Severity => {
  const text = sentimentView?.toLowerCase() ?? consensus?.replace('_', ' ').toLowerCase();
  if (!text) return 'unknown';
  if (text.includes('strong sell') || text.includes('sell')) return 'high';
  if (text.includes('hold') || text.includes('neutral')) return 'medium';
  return 'low';
};
...
export function RiskCard({ result, stock }: RiskCardProps) {
  ...
  const sentimentSeverity = categorizeSentiment(result?.sentimentView, sentiment?.analystConsensus);
  ...
  const riskFactors: RiskFactor[] = [
    {
      ...
      score: clamp01(typeof fundamentals?.debtToEquity === 'number' ? fundamentals.debtToEquity / 2 : 0.45),
      description: 'Debt-to-equity ratio supplied within fundamentals.',
    },
    {
      label: 'Volatility',
      value: typeof priceRangePct === 'number' ? formatPercent(priceRangePct) : 'N/A',
      severity: volatilitySeverity,
      score: clamp01(typeof priceRangePct === 'number' ? priceRangePct / 90 : 0.55),
      description: 'Percentage spread between 52-week high and low.',
    },
    {
      label: 'Liquidity',
      value:
        typeof volumeRatio === 'number'
          ? `${volumeRatio.toFixed(2)}x avg vol`
          : 'N/A',
      ...
      score:
        typeof volumeRatio === 'number'
          ? volumeRatio >= 1.2
            ...
            : 0.85
          : 0.55,
      description: 'Latest volume compared with trailing average.',
    },
    {
      label: 'Sentiment',
      ...
      score:
        sentimentSeverity === 'low'
          ? 0.35
          : sentimentSeverity === 'medium'
          ? 0.6
          : sentimentSeverity === 'high'
          ? 0.85
          : 0.5,
      description: 'Blend of narrative sentiment and analyst consensus.',
    },
  ];
  ...
  const sparklinePoints = riskFactors
    .map((factor, idx) => {
      const x = (idx / (riskFactors.length - 1 || 1)) * 100;
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-sm text-ai-muted">
              Visual breakdown of leverage, volatility, liquidity, and sentiment pressure behind the model&apos;s risk score.
            </p>
          </div>
          ...
          <div className="flex flex-1 flex-col gap-4 sm:flex-row sm:items-center sm:justify-end">
            <div className={cn('rounded-xl px-5 py-4 text-center min-w-[180px] shadow-inner', severityStyles[overallSeverity].bg)}>
              ...
              <div className="text-4xl font-bold text-ai-text mt-1">
                {Number.isFinite(riskScore) ? riskScore.toFixed(1) : '0.0'}
              </div>
              ...
              <div className="mt-3 h-2 bg-black/40 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${clamp01(riskScore / 10) * 100}%` }}
                />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2 text-sm text-ai-muted">
                ...
                Conviction {Number.isFinite(conviction) ? `${conviction.toFixed(0)}%` : 'N/A'}
              </div>
              <div className="flex items-center gap-2 text-sm text-ai-muted">
                <AlertOctagon className="h-4 w-4 text-yellow-400" />
                {result?.scenarios?.uncertaintyComment ?? 'No uncertainty callout provided.'}
              </div>
            </div>
          </div>
        </div>
                  ...
                  <TooltipTrigger asChild>
                    <span
                      className={cn(
                        ...
                        severityStyles[factor.severity].badge,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-ai-bg/50 border border-gray-800 rounded-lg p-4">
            ...
            {riskNotes.length > 0 ? (
              ...
              <p className="text-sm text-ai-muted">No specific risk alerts were surfaced for this ticker.</p>
            )}
          </div>
          ...
          <div className="bg-ai-bg/50 border border-gray-800 rounded-lg p-4 space-y-3">
            ...
            <p className="text-sm text-ai-text">
              ...
              risk with {Number.isFinite(conviction) ? `${conviction.toFixed(0)}%` : 'N/A'} short-term conviction. Combine
              the factor stress map with planning guidance before sizing positions.
            </p>
          </div>
        </div>
      </div>
